<template>
    <div v-if="target.length">
      <transition name="fade">
       ...
       <div class="notice_box2" v-show="target.length" >
       <div>
        <p>
            <i :class="{btn_reduce_number:true,left:true,btn_reduce_number_ok:count>=target[0].stepCount*2}" @click="count>=target[0].stepCount*2?count-=target[0].stepCount:0"></i>
        ...
        <p class="product_buy_info">
            ...
            <span class="right js_stock">{{target[0].stockCount}}</span>
        </p>
        <p class="product_buy_info">
            <span class="left">最小可拆零包装</span>
            <span class="right js_min">{{target[0].unit}}</span>
        </p>
        <p class="error_msg"></p>
    ...
    export default {
        ...
        watch: {
            count: function(newValue, oldValue) {
                ...
                if (newValue > this.target[0].stockCount) {
                    this.count = this.target[0].stockCount;
                } else if (newValue < this.target[0].stepCount) {
                    this.count = this.target[0].stepCount;
                } else if (newValue == '') {
                    this.count = 0;
                }
            },
        ...
        methods: {
            ...
            addShopCarList: function() {
                this.$http.post('/order/api/cart/addShopCart', {
                    "productCodeCompany": this.target[0].productCodeCompany,
                    "spuCode": this.target[0].spuCode,
                    "productId": this.target[0].productId,
                    "productCount":this.count,
                    "productPrice": this.target[0].productPrice,
                    "supplyId": this.target[0].vendorId,
                    "productName": this.target[0].productName,
                    "promotionId": this.target[0].mPromotionId,
                    "promotionCollectionId": this.target[0].mPromotionCollectionId,
                    "manufactures": this.target[0].factoryName,
                    "specification": this.target[0].spec
                }, {
                    headers: headers
                }).then(action => {
                    Toast({
