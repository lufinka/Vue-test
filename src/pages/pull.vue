        ...
        <div class="promotion-wrapper">
            <div class="promotion-title">
                <div class="promotion-tabs">
                    <ul>
                        <li v-for="(item,index) in promotionNav" :key="index" @click="active = 'tab-container'+(index+1)" :class="{actived:active == 'tab-container'+(index+1)}">
                            <p v-text="item.shortName || item.shopName"></p>
                        </li>
                    </ul>
                </div>
            ...
            <mt-tab-container v-model="active" swipeable>
                <mt-tab-container-item v-for="(item,index2) in promotionList" :key="index2" :id="'tab-container'+(index2+1)">
                    <div class="promotion-cont" ref="wrapper" :style="{ height: wrapperHeight + 'px' }">
                        <mt-loadmore :bottom-method="loadBottom" :autoFill="isfill" @bottom-status-change="handleBottomChange" :bottom-all-loaded="allLoaded[index2]" ref="loadmore">
                            <ul>
                                <li v-for="(p,n) in item" :key="n">
                                    {{item[n]}} {{bottomStatus[index2]}}
                                </li>
                            </ul>
                            ...
                            <div class="promotion-cont empty" v-if="empty[index2]">
                                <p>暂无数据</p>
                            </div>
                        </mt-loadmore>
                    </div>
